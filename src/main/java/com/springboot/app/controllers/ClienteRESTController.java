...
public class ClienteRESTController {
	...
	public ResponseEntity<?> update(@PathVariable Long id, BindingResult result, @RequestBody Cliente cliente)
	{
		Cliente cUpdated = null;
		Map<String, Object> response = new HashMap<>();

		Cliente cActual = clienteService.findById(id);
		if (cActual == null)
		{
			response.put("mensaje", "El cliente ID ".concat(id.toString().concat(" no existe en la BBDD")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		...
		try
		{
			...
			cActual.setCreateAt(cliente.getCreateAt());

			cUpdated = clienteService.save(cActual);
		}
		catch (DataAccessException e)
		{
	...
	public ResponseEntity<?> delete(@PathVariable Long id)
	{
		...
		{
			response.put("mensaje", "Error al realizar el DELETE en la BBDD");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
